defmodule Santorini.Pan do
  ...
  def winning_move(board, moves) do
    p = myplayer(board)
    cur_lvl = get_level(board.spaces, List.to_tuple(Enum.at(p.tokens, 0)))
    Enum.filter(moves, fn [i, j] -> get_level(board.spaces, {i, j}) == 3 or get_level(board.spaces, {i, j}) < cur_lvl-1  end)
  end
