defmodule Santorini.Minotaur do
  @moduledoc false
  ...
  def move(board, {xc, yc}, {xn, yn}) do
    ...
    tso = Enum.map(opponent.tokens, fn
  ...
  def possible_moves(board, {x, y}) do
    ...
    players = Enum.filter(players, fn [i, j] -> i == 1 or j == 1 or i == 5 or j == 5 end)
    # remove players from the edges
    moves = moves -- players
    moves
  end
