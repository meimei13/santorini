defmodule Santorini.Apollo do
  ...
  def move(board, {xc, yc}, {xn, yn}) do
    ...
    opponent = if Enum.any?(opponent.tokens, fn [i, j] -> Enum.any?(ts, fn x -> x == i or x == j end) end) do
      Enum.map(opponent, fn
        coord when coord == [xn, yn] -> [xc, yc]
        coord -> coord
      end)
    end
  ...
  def possible_moves(board, {x, y}) do
    ...
    # Do not remove spaces another player is on
    moves
  end
